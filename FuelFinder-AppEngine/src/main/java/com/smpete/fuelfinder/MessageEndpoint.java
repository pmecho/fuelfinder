...
/**
 * A simple Cloud Endpoint that receives notifications from a web client (<server url>/index.html), and broadcasts them
 * to all of the devices that were registered with this application (via DeviceInfoEndpoint).
 * <p/>
 * In order for this sample to work, you have to populate the API_KEY field with your key for server apps. You can
 * obtain this key from the API console (https://code.google.com/apis/console). You'll first have to create a project
 * and enable the Google Cloud Messaging Service for it, as described in the javadoc for GCMIntentService.java (in your
 * Android project).
 * <p/>
 * After creating the project in the API console, browse to the "API Access" section, and select the option to "Create a
 * New Server Key". The generated key is what you'll enter into the API_KEY field.
 * <p/>
 * See the documentation at http://developers.google.com/eclipse/docs/cloud_endpoints for more information.
 * <p/>
 * NOTE: This endpoint does not use any form of authorization or authentication! If this app is deployed, anyone can
 * access this endpoint! If you'd like to add authentication, take a look at the documentation.
 */
@Api(name = "messageEndpoint", namespace = @ApiNamespace(ownerDomain = "smpete.com", ownerName = "smpete.com", packagePath = "fuelfinder"))
...
public class MessageEndpoint {
     ...
     */
    private static final String API_KEY = "AIzaSyCB4rXe24ZAhBeOUzpEeFNT7wCuY6UarB4";

    private static final DeviceInfoEndpoint endpoint = new DeviceInfoEndpoint();

    /**
     * This function returns a list of messages starting with the newest message first and in descending order from
     * there
     *
     * @param cursorString for paging, empty for the first request, subsequent requests can use the returned information
     *                     from an earlier request to fill this parameter
    ...
    /**
     * This accepts a message and persists it in the AppEngine datastore, it will also broadcast the message to upto 10
     * registered android devices via Google Cloud Messaging
     *
