...
/**
 * A copy of the framework's {@link android.content.CursorLoader} class. Copied because CursorLoader is not
 * parameterized, and we want to parameterize over the underlying cursor type.
 *
 * @param <T>
 */
public class ObjectCursorLoader<T> extends AsyncTaskLoader<ObjectCursor<T>> {
    ...
    /**
     * Starts an asynchronous load of the contacts list data. When the result is ready the callbacks will be called on
     * the UI thread. If a previous load has been completed and is still valid the result may be passed to the callbacks
     * immediately.
     * <p/>
    ...
    /**
     * For debugging loader-related race conditions. Delays the background thread load. The delay is currently run after
