...
package com.smpete.fuelfinder.utils;
...
/**
 * A cursor-backed type that can return an object for each row of the cursor. This class is most useful when: 1. The
 * cursor is returned in conjuction with an AsyncTaskLoader and created off the UI thread. 2. A single row in the cursor
 * specifies everything for an object.
 */
public class ObjectCursor<T> extends CursorWrapper {
    ...
    /**
     * Create a concrete object at the current cursor position. There is no guarantee on object creation: an object
     * might have been previously created, or the cache might be populated by calling {@link #fillCache()}. In both
     * these cases, the previously created object is returned.
     *
     * @return a model
     */
    ...
    /**
     * Reads the entire cursor to populate the objects in the cache. Subsequent calls to {@link #getModel()} will return
     * the cached objects as far as the underlying cursor does not change.
     */
